...
class OmniFMDrDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
    ...
    def __getitem__(self, index):
        ...
        # print(f"********type3: {type(patch_image), patch_image.shape}")
        # exit()
    ...
    def random_augmentation(self, im, boxes):
        ...
        if random_angle != 0:
            ...
            x, y, w, h = copy.deepcopy(boxes[:, 0]), copy.deepcopy(boxes[:, 1]), copy.deepcopy(boxes[:, 2]), copy.deepcopy(boxes[:, 3])
            if random_angle == 90:
                boxes[:, 0] = y
